...
def get_qid_descriptions(qids):
    ...
    with urllib.request.urlopen(req) as resp:
        data = json.load(resp)
        return [(b["label"]["value"], b["description"]["value"]) for b in data["results"]["bindings"]]
...
to_sentence_prompt_template = """You will be given a label and a description for an entity from Wikidata. For each, think step-by-step about how to combine them into a natural, fluent English sentence. Then write the final sentence, wrapped in <answer>...</answer> tags. Do not include any extra commentary.
...
batch_size = 100
completed_file = "completed_descriptions"
out_file = "data/descriptions/descriptions.log"

@torch.no_grad()
def iteration(xs, i):
    global completed, batch_size
    
    log(f"starting batch {i}")
    
    descs = get_qid_descriptions([x['qid'] for x in xs])

    log(f"fetched descriptions for batch {i}")
    
    if len(descs) == 0:
        log(f"batch has no descriptions, skipping")
        return
    
    ans = prompt_model([to_sentence_prompt_template % result for result in descs], 300, True)

    log(f"generated sentences for batch {i}")
...
iterate_labels(iteration, batch_size, offset=completed)